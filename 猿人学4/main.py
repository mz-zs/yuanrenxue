...
for page in range(1, 6):
    ...
    key = dict(re.json())['key']
    value = dict(re.json())['value']
    with open('play.js', 'r') as f:
        play_code = f.read()
    play = execjs.compile(play_code)
    display_code = play.call('play', key, value)

    info_data = dict(re.json())['info']

    etree_html = etree.HTML(info_data)

    td_list = (etree_html.xpath('//td'))
    i = 0
    data_dict = dict()
    for td in td_list:
        i += 1
        ...
        for img in img_list:
            src = img.xpath('./@src')[0]
            src = image_dict[src]
            _class = img.xpath('./@class')[0]
            _class = _class.split()[1]
            style = img.xpath('./@style')[0]
            style = float(style.split(':')[1][:-2])
            if _class != display_code:
                data_dict[f'data{i}'].append([src, style])

    num = []
    for i in range(1, len(data_dict)+1):
        power = len(data_dict[f'data{i}'])
        temp_num = 0
        for j in range(0, power):
            temp_num = temp_num + (data_dict[f'data{i}'][j][0])*10 ** ((power-j-1-int(data_dict[f'data{i}'][j][1]/11.5)))
        num.append(temp_num)
